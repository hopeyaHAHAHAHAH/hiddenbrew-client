...
document.addEventListener("DOMContentLoaded", () => {
    const checkoutListContainer = document.querySelector(".checkout-list");
    ...
    if (cart.length === 0) {
        ...
        totalPriceElement.textContent = "P 0.00";
        return;
    }
    ...
    cart.forEach((item) => {
        ...
        itemElement.innerHTML = `
            ...
            <div class="quantity-price">P ${(item.price * item.quantity).toFixed(2)}</div>
        `;
        checkoutListContainer.appendChild(itemElement);
